...
src/
  ...
  pages/
    ...
    H2H.jsx
    QuoiDeNeuf.jsx
    Admin.jsx               ← NOUVEAU v3.1 — modération messages
  components/
...
- Connecté : fond semi-transparent, "Content de te revoir + pseudo", message contextuel pronos en attente si applicable, bouton C'est parti
...
3. **TIMELINE** — BandeMatchs + bouton Calendrier
4. **Le Vestiaire** — fond beige `#f0ede8`, streaks potes + chat ligues
5. Bannière Unsplash
6. StandingsNBA + BracketPlayoffs (si playoffs)
7. **ACTU NBA** — NewsNBA

**Bloc vide BandeMatchs :** si aucun match dans les 3 jours, affiche un message avec lien calendrier.

### Focus.jsx ✅ — v3.1
- Carousel séquentiel avec fade (350ms) toutes les 4s
- Messages dans l'ordre :
  1. 🏀 Matchs ESPN sans prono (depuis `nbPronosAttente` prop)
  2. ⏳ Pronos Supabase `en_attente` (matchs posés mais pas encore joués)
  3. 🔥 Streak correct ≥ 2
  4. 💪 Streak incorrect ≥ 2
  5. 💔 Série cassée (streak actuel = 1 incorrect après série ≥ 2 corrects)
  6. 🎯 Win rate (si ≥ 5 pronos résolus)
- `nbPronosAttente` reçu en prop depuis `Accueil.jsx` (calculé dans `BandeMatchs`)
- Se masque si aucun message

### Badge nav "pronos en attente" ✅ — v3.1
- Point rouge sur l'icône Board (mobile bottom nav + desktop top nav)
- Calculé dans `BandeMatchs` : matchs pas terminés, pas en cours, pas encore pronostiqués
- Remonté via prop `onBadge` → `Accueil.jsx` → `Navigation.jsx`
- Disparaît dès que tous les matchs pronosticables sont faits

### Le Vestiaire ✅ — v3.1
- Streaks potes (correct ≥ 2, incorrect ≥ 2, série cassée ≥ 2)
- Chat par ligue en cours : 3 derniers messages + input inline
- Polling 30s sur les messages
- Date/heure sur chaque message : format `jj/mm hh:mm` (UTC→local avec `+ 'Z'`)
- Chaque user voit les streaks des autres, pas le sien (le sien est dans Focus)

### Admin.jsx ✅ — v3.1 (NOUVEAU)
- Page `/admin` — accessible uniquement via `ADMIN_ID`
- Fil complet de tous les messages de toutes les ligues (100 derniers)
- Bouton 🗑️ suppression par message
- Lien dans le menu hamburger visible uniquement si admin
- Redirection `/accueil` si user non admin

### PopupChangelog.jsx ✅ — v3.1
- "Content de te revoir, [pseudo] !" sous le logo
- Message contextuel : pronos en attente uniquement (fallback "Bon retour" supprimé)

### NewsNBA.jsx ✅ — v3.1
- Fix : plus de dépendance à `typeSaison` — les news chargent toujours
- `useEffect` avec dépendance `[]` (une seule fois au montage)
...
- Prédiction ESPN (win probability) si match non terminé

### Autres pages ✅ — toutes refondues v3.0
- `Groupes.jsx`, `Profil.jsx`, `H2H.jsx`, `Calendrier.jsx`, `BracketPlayoffs.jsx`, `ClassementRapide.jsx`, `QuoiDeNeuf.jsx`

---

## 9. Décisions produit

### Session 2026-06-02 — Refonte charte complète (v3.0)
...
- **Stats MatchDetail** : barres horizontales bicolores couleurs ESPN
- **MVP ex-aequo** : pas de gagnant enregistré si égalité

### Session 2026-06-02 — Sprint 3 features (v3.1)
- **Badge nav** : point rouge sur icône Board — calculé depuis BandeMatchs, pas depuis Supabase
- **Focus enrichi** : reçoit `nbPronosAttente` en prop, gère aussi la série cassée perso
- **Le Vestiaire** : chat ligues en cours intégré inline dans le Board — pas de page dédiée
- **Admin** : page `/admin` pour modération — suppression SQL directe aussi possible
- **Messages RLS** : GRANT explicite `SELECT, INSERT, DELETE TO authenticated` requis en plus des policies
- **NewsNBA** : `typeSaison` retiré — les news s'affichent indépendamment de l'état ESPN
- **BandeMatchs vide** : bloc informatif avec lien calendrier si aucun match 3 jours
- **Popup welcome back** : pseudo affiché, message contextuel uniquement si pronos en attente
- **Pronostic écart final** : reformulation "score exact" adaptée au basket (victoire <5 pts / >20 pts = +2 pts) — mis de côté, post-Sprint 4
- **Score exact** : impossible au basket (scores trop élevés) — supprimé du roadmap

### Features futures notées (post-Sprint 4)
- H2H historique équipes en saison régulière dans MatchDetail (distinct du H2H playoff déjà dispo)
...
|---|---|---|
| 1 | matchs ESPN sans prono > 0 | "Il te reste N match(s) à pronostiquer !" |
| 2 | pronos Supabase en_attente > 0 | "Tu as N pronostic(s) en cours !" |
| 3 | streak correct ≥ 2 | "Tu es sur une série de N pronos réussis" |
| 4 | streak incorrect ≥ 2 | "N ratés d'affilée..." |
| 5 | série cassée (streak=1 incorrect après ≥2 corrects) | "Tu viens de briser ta série de N..." |
...
| Série cassée | streak précédent ≥ 2 correct, dernier = incorrect | "💔 Pseudo vient de briser sa série de N..." |
...
### DETTE-16 — Bracket/Standings : affichage basé sur `typeSaisonActuel` ESPN (null si pas de matchs 3 jours)
**Sévérité :** 🟡 Moyenne — à corriger en basant l'affichage sur le `type_saison` des ligues en cours de l'user

### DETTE-17 — Forme récente MesPronos : ordre à vérifier (dernier match doit être à droite)
**Sévérité :** 🟢 Faible — à corriger Sprint 3
...
✅ Focus enrichi (nbPronosAttente + série cassée perso)
✅ Le Vestiaire — fin de série (détection série cassée potes)
...
⏳ Onboarding simplifié (pitch → premier prono → notifs)
⏳ Leaderboard global
⏳ Partage de pick (Canvas API, format Story)
⏳ Bracket/Standings — logique basée sur type_saison ligues en cours
⏳ Forme récente — ordre (dernier à droite, max 5, glisse dans le temps)
⏳ Actus NBA en français (source à trouver)
⏳ Focus → Briefing — renommer + messages guideline cliquables avec localStorage dismiss
```
...
- Pronostic écart final — post-Sprint 4
...
Clé `anon` Supabase : publique par design — la sécurité repose sur les RLS, pas sur la clé.
Clé `service_role` : jamais dans le front — uniquement variables d'environnement Vercel.
...
- `messages` : croissance dépend de l'activité — très légère à ce stade
...
|---|---|---|
| `socle_nba_v3_1.md` | Référence technique | ✅ Ce document |
| `swish_league_roadmap_v1_4.md` | Vision produit, roadmap | ✅ Actif |
| `espn_capacites_v1_0.md` | Cartographie ESPN API | ✅ Actif |

---

*Document v3.1 — 2026-06-02*
*Remplace socle_nba_v3_0.md*
