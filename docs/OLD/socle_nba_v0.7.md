...
- **NBA Cup = type 2** (saison régulière) — ESPN ne la distingue PAS. Filtre type 4 inutile, option supprimée.
...
Migration effectuée le 2026-05-27 — ancienne contrainte (user_id, match_id) supprimée.
...
Fonction partagée `recupererLiguesCibles(userId, typeSaisonMatch)` — dupliquée dans `Accueil.jsx` et `MatchDetail.jsx`.
...
✅ Scores par quart-temps — fix 2026-05-27 (clé `displayValue` au lieu de `value`)
✅ Lieu + stade + ville + date/heure
✅ Forme récente L5
✅ Stats du match (matchs terminés) ou stats moyennes saison (matchs à venir) — fix 2026-05-27
✅ Leaders avec photo
✅ Blessés/Absents
...
✅ Quitter (soft delete)
⚠️ Type 4 (NBA Cup) supprimé — ESPN code ces matchs en type 2 (saison régulière)
