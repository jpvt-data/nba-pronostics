...
### groupes
`id` | `nom` | `admin_id` | `date_fin` (date, nullable) | `type_saison` (int) | `saison` (int)
...
### pronos
`id` | `user_id` | `match_id` | `equipe_choisie` | `resultat` ('en_attente'|'correct'|'incorrect') | `points_gagnes` | `cree_le`
...
✅ Toggle Board + hamburger + mobile logo bar
...
✅ No Spoil : score masqué, perdant non grisé
...
✅ Champs : nom, type ESPN (1-5), saison ESPN, date de clôture optionnelle
