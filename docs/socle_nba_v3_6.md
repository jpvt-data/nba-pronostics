...
### Tables actuelles
`profils` | `groupes` | `membres_groupe` | `matchs` | `pronos` | `semaines_gagnees` | `messages` | `xp_log` | `missions` | `missions_utilisateurs` | `badges_catalogue`
...
src/
  ...
  data/
    changelog.js
    badges.js            ← BADGES_CATALOGUE + badgeImageUrl() — source de vérité badges
  services/
    espn.js
    points.js            ← calculerPoints() + verifierJalons() + semaine 100%
    ligues.js
  ...
  pages/
    ...
    MesPronos.jsx        ← header XP/badges, popup badge, historique XP, modal info
    MatchDetail.jsx      ← XP anti-doublon branché
    Calendrier.jsx
    Profil.jsx
    Stats.jsx
    H2H.jsx
...
- **Utilisateurs** (nouveau) : select dropdown alphabétique, attribution/retrait badges manuels avec feedback
- Modération

### MesPronos (Stats) ✅ — v3.5
...
- Modal ℹ️ : 3 onglets (XP & Niveaux / Badges mystérieux / Missions)
- Modal Historique XP : 100 derniers gains triés par date, labels lisibles, visible uniquement par l'user
...
Nouvelles notifications : jalons XP < 24h, badges récemment obtenus, changement de niveau, changement de titre RPG. Tous les messages existants conservés.
...
#### Badges — catalogue définitif dans `src/data/badges.js`
...
| `en_hibernation` | En Hibernation | performance | Auto (5 ratés) |
| `en_feu` | En Feu | performance | Auto (5 corrects) |
...
| `prophete` | Prophète | performance | Auto (10 corrects) |
...
**Règle d'attribution manuelle (admin) — anti-doublon :**
```sql
UPDATE profils
SET badges = array_append(badges, 'le_sacre')
WHERE id = 'uuid-du-gagnant'
AND NOT ('le_sacre' = ANY(badges));
```

**Badges manuels dans Admin :** filtrés par `famille === 'appartenance' || famille === 'evenement'`.
Badges `performance` = automatiques uniquement, ne doivent pas apparaître dans l'Admin.
...
Noté pour post-Sprint 4 — sécuriser l'attribution XP côté serveur pour éviter les manipulations. Non urgent pour usage entre amis avec RLS actif.

#### Répartition des points
À revoir avant juillet 2026 (Summer League). Actuellement : 1 point par prono correct.
...
⏳ Assets badges — upload WebP dans Supabase Storage (en cours)
⏳ Audit XP en fin de Finals — vérifier jalons, semaine_100_pct, prono_correct pour chaque user
```

### Avant juillet 2026 (Summer League)
```
⏳ Répartition des points — à revoir (actuellement 1 pt par correct)
...
|---|---|---|
| `socle_nba_v3_6.md` | Référence technique unique | ✅ Ce document |
| `swish_league_roadmap_v1_9.md` | Vision produit, roadmap | ✅ Actif |
| `espn_capacites_v1_1.md` | Cartographie ESPN API | ✅ Actif |

---

*Document v3.6 — 2026-06-05*
*Remplace socle_nba_v3_5.md*
