...
- `recupererGagnant()` dans `espn.js` retourne maintenant `score_domicile` et `score_exterieur`
...
### BandeMatchs ✅ — v3.9
- Flèches navigation desktop `‹` `›` apparaissant au hover — navigation groupe par groupe
- Flèches masquées sur mobile (≤768px) — scroll tactile inchangé
- Positionnement initial centré sur le prochain match à jouer
...
- `recupererGagnant()` retourne `score_domicile` + `score_exterieur` — stockés dans `matchs` via `calculerPoints()`
- `semaine_100_pct` : attribution uniquement le lundi heure Paris, évalue la semaine précédente (lundi-7 → dimanche-1)
- Fourchette d'écart : résolution immédiate si match déjà terminé au moment de la pose (`ecart.js`)
- `BandeMatchs` : flèches desktop navigation groupe par groupe, masquées mobile
- `Briefing` : ticker démarre plein, boucle sans gap, libellés missions contextualisés par slug

#### Nouveaux chantiers identifiés
- Filtrage pronos saison régulière (1230 matchs/saison)
- Cotations matchs (ESPN + Odds API FR)
- Profil fan confirmé Sprint 4
...
**Sévérité :** 🟡 Moyenne — l'attribution ne se fait que si un user se connecte le lundi. Si personne ne se connecte ce jour-là, la semaine est perdue. Fix définitif : Edge Functions Supabase (post-Sprint 4).
...
⏳ Cotations matchs — intégrer ESPN data.odds + explorer Odds API pour cotes FR (Unibet, Winamax) — post-Sprint 4, note légale ANJ
```
...
Filtrage pronos saison régulière — mécanisme à concevoir (1230 matchs/saison, invivable sans filtre) — pistes : équipe favorite, conférence, top matchs ESPN
...
|---|---|---|
| `socle_nba_v3_9.md` | Référence technique unique | ✅ Ce document |
| `swish_league_roadmap_v2_2.md` | Vision produit, roadmap | ✅ Actif |
| `espn_capacites_v1_1.md` | Cartographie ESPN API | ✅ Actif |

---

*Document v3.9 — 2026-06-07*
*Remplace socle_nba_v3_8.md*
