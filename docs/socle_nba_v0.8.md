...
⚠️ `LabelSection`, `BanniereImage` et `BLOC` sont encore dupliqués dans chaque page — extraction dans `components/UI.jsx` prévue Phase 3.

### Règle headers
- Accueil, Classement, Calendrier, Groupes : header plein bord
...
src/
  ...
  pages/
    ...
    MesPronos.jsx      — stats perso + publiques via ?user_id, forme récente W/L
                         ↳ Forme récente triée par date_match (pas par date de création du prono)
  ...
  components/
    Navigation.jsx     — desktop top + mobile logo top + mobile bottom nav + hamburger
                         ↳ Consomme ProfilContext — aucun fetch Supabase au montage
    Avatar.jsx         — composant Avatar + couleurAvatar — partagé par Navigation, MesPronos,
                         Classement, Profil
    BandeMatchs.jsx    — bande scrollable horizontale (3 jours) + No Spoil
                         ↳ Pronos chargés filtrés sur les matchs affichés uniquement
...
✅ Avatar visible dans Navigation (desktop + mobile + hamburger) via ProfilContext
...
✅ Calcul points au chargement Board — ESPN appelé une seule fois par match (déduplication)
...
✅ Création réservée à l'admin (ADMIN_ID hardcodé côté client)
✅ Modification ligue : nom, type, saison, date de clôture — admin uniquement
✅ Suppression ligue : supprime membres_groupe + groupe — admin uniquement, confirmation requise
✅ Champs : nom, type ESPN (1/2/3/5 ou null=général), saison ESPN, date de clôture optionnelle
✅ Clôture automatique si date_fin dépassée
✅ Tri automatique : actives en haut, terminées en bas
✅ Badge "✓ Inscrit", points affichés
✅ Quitter (soft delete)
⚠️ Type 4 (NBA Cup) absent — ESPN code ces matchs en type 2 (saison régulière)
⚠️ Filtre "NBA Cup" encore présent dans Calendrier.jsx — à nettoyer (Phase 3)
...
⚠️ Stats (taux de réussite) calculées sur tous les pronos de l'user, toutes ligues confondues — à affiner Phase 3
...
- En début de session : uploader uniquement les fichiers modifiés depuis la dernière session (`git diff --name-only HEAD~1`)

## Backlog Sprint 1 (restant)
- [ ] Sélecteur de saison dans Calendrier
- [ ] Table equipes ESPN directe (remplace table Supabase prévue — données ESPN toujours à jour)
- [ ] ADMIN_ID hardcodé côté client → sécuriser côté serveur en prod
- [ ] Calcul points côté serveur (Edge Function) — actuellement déclenché au chargement Board
- [ ] Leaders stats NBA via proxy Supabase Edge Function (CORS bloqué)
- [ ] Filtre "NBA Cup" dans Calendrier.jsx à supprimer
