...
// Couleurs d'avatar par initiale (fallback si pas de photo)
...
function ChampEditable({ label, valeur, onSave, multiline = false, placeholder = '' }) {
  ...
  if (edit) return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
        <button onClick={sauver} disabled={saving} style={{
  ...
  return (
    <div onClick={() => setEdit(true)} style={{ cursor: 'pointer', group: true }}>
      ...
      <div style={{
        ...
        padding: '8px 10px',
        borderWidth: 1, borderStyle: 'dashed', borderColor: 'transparent',
...
function Profil() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const { data: membres } = await supabase
        .from('membres_groupe')
        .select('points, groupes(id, nom, date_fin)')
        .eq('user_id', user.id)
        .eq('actif', true)
  ...
  const changerAvatar = async (e) => {
    ...
    // Upload dans bucket avatars — nom = user id (écrase l'ancien)
  ...
  return (
    <>
      <Navigation />
      <main style={{ flex: 1, padding: '20px 16px 40px', maxWidth: 520, margin: '0 auto', width: '100%' }}>
        ...
        <div style={{
          ...
          <div style={{ position: 'relative', flexShrink: 0 }}>
            {profil?.avatar_url
              ? <img src={profil.avatar_url} alt={profil.pseudo}
                  style={{ width: 72, height: 72, borderRadius: '50%', objectFit: 'cover', background: 'var(--bg-2)' }} />
              : <div style={{
                  width: 72, height: 72, borderRadius: '50%',
                  background: couleurAvatar(profil?.pseudo),
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
            ...
            <button
              ...
              style={{
                ...
                borderWidth: 2, borderStyle: 'solid', borderColor: 'var(--bg-1)',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
        ...
        <div style={{
          ...
          <ChampEditable
            label="Pseudo"
            valeur={profil?.pseudo}
            placeholder="Ton pseudo"
            onSave={(v) => sauverChamp('pseudo', v)}
          />
          <div style={{ borderTopWidth: 1, borderTopStyle: 'solid', borderTopColor: 'var(--border)' }} />
          <ChampEditable
            label="Ta bio"
            valeur={profil?.description}
            placeholder="Dis un truc sur toi… ton équipe favorite, ton niveau de trash talk, etc."
            multiline
            onSave={(v) => sauverChamp('description', v)}
          />
        </div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 11, color: 'var(--text-3)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 16 }}>
            Stats pronos
          </div>
